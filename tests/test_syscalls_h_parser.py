...
@pytest.mark.parametrize(
    "syscalls_h_content,entry_name,expected",
    [
        (
        ...
        ("asmlinkage long sys_getpid(void);", "sys_getpid", SyscallSignature(name="getpid", parameters=[])),
        (
...
def test_parse_syscall_signature_multiline():
    ...
    """
    expected = SyscallSignature(
        name="openat", parameters=["int dfd", "const char *filename", "int flags", "umode_t mode"]
    )
